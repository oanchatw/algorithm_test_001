...
export default defineComponent({
  name: 'ProblemView',
  components: { CodeBlock, DifficultyBadge },
  setup() {
    const route = VueRouter.useRoute();
    const chapter = computed(() => getChapter(Number(route.params.chapterId)));
    const problem = computed(() => {
      return chapter.value?.problems?.find(p => p.id === Number(route.params.problemId));
    });
    ...
    return { chapter, problem, prevProblem, nextProblem };
  },
  template: `
    <div class="problem-view" v-if="problem">
      <div class="problem-header">
        <router-link :to="'/chapter/' + chapter.id" class="back-link">← Ch{{ chapter.id }}: {{ chapter.title }}</router-link>
        <h1>{{ problem.title }}</h1>
      ...
      <div class="problem-nav">
        <router-link v-if="prevProblem" :to="'/chapter/' + chapter.id + '/problem/' + prevProblem.id" class="nav-btn">← {{ prevProblem.title }}</router-link>
        <router-link v-if="nextProblem" :to="'/chapter/' + chapter.id + '/problem/' + nextProblem.id" class="nav-btn">{{ nextProblem.title }} →</router-link>
      </div>
    </div>
